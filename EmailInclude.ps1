...
#	Creates the email Body depending on the email "type"	
	...
	switch ( $btype ) {
		...
		'SF Many Files'		{ 
			...
			$outBody = $outBody + ' check the Check GoAnywhere Service for'
			$outBody = $outBody + ' SchoolsFirst Posting Files.<br>'
			$outBody = $outBody + ' \S_WebDocs\schoolsfirst\ folder on'
			$outBody = $outBody + ' DC-FS01.DC.BLUEPOINT.COM!<br></font>'
		}
		'SF Many Posting Files 90'		{ 
			$outBody = '<font color="red">There is an ERROR SchoolsFirst posting'
			$outBody = $outBody + ' file on DC-App90.<br> Please check the'
			$outBody = $outBody + ' C:\Program Files (x86)\Bluepoint Solutions\QwikDeposit Post\ErrorFiles\'
			$outBody = $outBody + '	folder on'
			$outBody = $outBody + ' DC-App90.DC.BLUEPOINT.COM!<br></font>'
		}
		'SF Many Posting Files 91'		{ 
			$outBody = '<font color="red">There is an ERROR SchoolsFirst posting'
			$outBody = $outBody + ' file on DC-App91.<br> Please check the'
			$outBody = $outBody + ' C:\Program Files (x86)\Bluepoint Solutions\QwikDeposit Post\ErrorFiles\'
			$outBody = $outBody + '	folder on'
			$outBody = $outBody + ' DC-App91.DC.BLUEPOINT.COM!<br></font>'
		}		
		'SF Many Posting Files 92'		{ 
			$outBody = '<font color="red">There is an ERROR SchoolsFirst posting'
			$outBody = $outBody + ' file on DC-App92.<br> Please check the'
			$outBody = $outBody + ' C:\Program Files (x86)\Bluepoint Solutions\QwikDeposit Post\ErrorFiles\'
			$outBody = $outBody + '	folder on'
			$outBody = $outBody + ' DC-App92.DC.BLUEPOINT.COM!<br></font>'
		}
		'Many Files'		{ 
			$outBody = '<font color="red">File count is over the threshold please'
		...
		'SF No Files'		{ 
			$outBody = '<font color="red">Unable to conect to the SchoolsFirst'
			$outBody = $outBody + ' FTP folder at this time.<br></font>'
		}
		'SF No Posting Files'		{ 
			$outBody = '<font color="red">Unable to conect to the SchoolsFirst'
			$outBody = $outBody + ' posting folder at this time.<br></font>'
		}
		'Files Recovery'		{ 
		...
		'SF Files Recovery'		{ 
			$outBody = "The monitoring system is now able to connect to the" 
			$outBody = $outBody + " SchoolsFirst FTP folder.<br>The monitoring"
			$outBody = $outBody + " system has recovered successfully."
		}
		'SF Posting Files Recovery'		{ 
			$outBody = "The monitoring system is now able to connect to the" 
			$outBody = $outBody + " SchoolsFirst Posting folder.<br>The"
			$outBody = $outBody + " monitoring systemhas recovered successfully."
		}
		default			{
