...
foreach($OU in $OUs){
    ...
    if($ComputerName){
        foreach($computer in $ComputerName){
           $computer | Get-InstalledSoftware | Where-Object Name -like "Microsoft SQL Server 2012*" | Select-Object ComputerName,Name,Version | Export-Csv -Append -Path "SW.CSV" -force
        }
    } 
}
